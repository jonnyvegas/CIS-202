...
PolyPtr head, second_head, third_head, fourth_head;
...
void Polynomial::output(Polynomial *&the_head)
{
    ...
    do
    {
        ...
        for(int i = num_of_terms; i > 1; i--)//step through until we are at the term we want to print
                                             //since it needs to print backwards.
        {
            temp = temp->get_link();
        }
        ...
        temp = the_head;
        for(int i = num_of_terms; i > 2; i--)//the next number in the list
        {
            temp = temp -> get_link();
        }
        if(temp->get_coefficient() < 0)
        ...
        if ((negative == false) && (last == false))//next number is negative and we are not on the head number in the list
        {
            cout << "+";
        }
        num_of_terms--;
